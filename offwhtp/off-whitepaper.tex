...
\begin{document}
	...
	\section{Imagining Bitcoin as a Computer}
		Ethereum utilizes the crypto-economic ledger model that originated with Bitcoin and repurposes it to model not merely a ledger, but a virtual machine, complete its own byte code. Ethereum allows machine code to be executed with the same certainty and verifiability as Bitcoin transactions. Just as sure as you can be certain of a Bitcoin transaction being the right amount, and that things in the record are timestamped and actually \textit{did in fact happen}, just as certain as the 1st, 2nd, 1000th, and 10,000th transactions on the Bitcoin network are the same transactions that happened nine years ago, (for no alteration of the record is possible, nor has ever been possible) just so sure are the \textit{machine instructions in bytecode that are given to the Ethereum interpreter.} 			     \index{Bitcoin}
		...
		This has profound implications for all areas of society although this would be the wrong context to explore those implications. But in short, \textit{programs written and executed on the Ethereum Blockchain are unstoppable.} This does not mean that no loopholes in the code exist that could cause undesirable outcomes, rather it means that at the base level, the outcomes, whether desirable or not, \textbf{happened}, \textbf{remain verifiable}, \textbf{are recorded}, and are \textbf{entirely  uncensorable}. 
		\index{unstoppable}
		\index{loopholes}
...
\justify
	...
	\section{Memory and Storage}
			...
			\subsection{World State}
				The actual state of the Ethereum Blockchain, after all transactions have been finalized and applied, this is a \textsc{mapping} of \textbf{addresses} and \textbf{account states} through the use of RLP. The mapping is stored as a Merkle-Patricia trie in a \textsc{database backend}.\footnote{The database backend is accessed by users through an external application, most likely an Ethereum client; see also: \gls{state database}} that maintains a mapping of bytearrays to bytearrays.\footnote{A bytearray is specific set of bytes [data] that can be loaded into memory. It is a structure for storing binary data, e.g. the contents of a file.} The cryptographic internal data going back to the root node represents the \textit{State} of the Blockchain at any given root, i.e. at any given \textit{time}.\footnote{This permanent data structure makes it possible to easily recall any previous state with its root hash keeping the resources off-chain and minimizing on-chain storage needs.}As a whole, the state is the sum total of database relationships in the \textbf{ \gls{state database}}. The state is an inert position on the chain, a position between prior state and post state; a block's frame of reference, and a defined set of relationships to that frame of reference.
			\index{World State}
			\index{mapping}
			\index{RLP}
...
\subsection{Blocktree to Blockchain}
		...
		\subsection{Ethash}	\index{asic resistant}
					\index{ethash}
					\index{mix hash}
